...
class MenuDashboardLeft extends Component{
    ...
    showMenuUser(){
        ...
        if(user && user.role && user.role.name === 'Administrator') {
            return(
                <li>
                    <Link to="/dashboard/users" className={(this.props.location.pathname == '/dashboard/users') ? 'tz-lma' : ''} ><i className="address card outline icon" />Usuários</Link>
                </li>
            )
        }
    }
    ...
    showHideMenu(){
        if(this.state.hideMobile)
            this.setState({hideMobile:false})
        else
            this.setState({hideMobile:true})

        console.log('stateeee: ', this.state.hideMobile);
    }
    ...
    render(){
        ...
        return(
            <div className="tz-l">
            <div className="tz-l-1 hidden-mobile">
                <ul>
            ...
            <ul>
                ...
                <li class={this.state.hideMobile ? 'show-mobile hidden-desktop': 'hidden-mobile hidden-desktop'} >
                    <a href="javascript: void(0)" onClick={e => this.showHideMenu()}><i className="fa fa-tachometer"></i> Exibir Menu</a>
                </li>
                <div class={this.state.hideMobile ? 'hidden-mobile': ''}>
                    <li>
                        <Link to="/dashboard" onClick={e => this.showHideMenu()} className={(this.props.location.pathname == '/dashboard') ? 'tz-lma' : ''}><i className="fa fa-tachometer"></i> Minha Dashboard</Link>
                    </li>
                    <li>
